...
**/
  ...
  if ($_SESSION[$project_pre]['type']=='superadmin'){
    ...
    $cascade   = $mysqli->query("SELECT id FROM `".$GLOBALS['TBL']['PREFIX']."cities` WHERE state_id=$id");
    if ($cascade->num_rows > 0){
      $error_message = "There are associated rows in Cities Table. ";
