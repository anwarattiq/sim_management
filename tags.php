...
if ($action == "update_$module" || $action == "add_$module") {
	$tag				= $mysqli->real_escape_string(stripslashes($_POST['tag']));
} else {
	$tag				= '';
}
...
if ($action == "update_$module" && !empty($id)) {
	...
	if (empty($tag) || $tag == 'Please select') {
		$error_message = 'Tag is mandatory.';
	} else if (checkDuplicateRow($tbl_name, 'tag', $tag)) {
		$error_message = 'Duplicate Tag. Please enter different.';
	} else {
	...
	if (empty($tag) || $tag == 'Please select') {
		...
		$insert_row = $mysqli->query("INSERT INTO `$tbl_name`(tag, publish) VALUES ('" . $tag . "', '" . $publish . "'); ");
						...
						<div class="panel-heading">
							...
							<div class="panel-body">
								...
								<div class="form-group">
									<label class="col-lg-3 control-label"><strong>Tag</strong> <span class="f_req">*</span></label>
									<div class="col-lg-9">
										<input name="tag" id="tag" value="<?php echo $tag; ?>" class="form-control" type="text">
									</div>
								</div>
