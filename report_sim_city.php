...
<div class="content-wrapper">
	<div class="page-header page-header-default">
		<div class="page-header-content">
			<div class="page-title">
				<h4><i class="icon-circle-right2 position-left"></i> Listing <?php echo '<strong>' . ucwords('Report City wise') . '</strong>'; ?> <?php echo $module_caption; ?></h4>
		...
		<div class="row">
			<div class="col-md-12">
			 		...
			 		<div class="form-group">
										<label class="col-lg-3 control-label"><strong>Select City Name</strong><span class="f_req">*</span></label>
										<div class="col-lg-9">
											<select class="form-control" name="city" id="city" onchange="function_city(this.value);">
   ...
   <script type="text/javascript">
     function function_city(cid){
        
        window.location.href ="https://shopinitools.com/sim/report_sim_city.php?cid="+cid; 
    ...
    $result_sims = $mysqli->query("SELECT sum(amount)as amount,sum(`mt_invoice`.qty) as qty,company_name FROM `mt_invoice` inner join `mt_sims_request` on `mt_sims_request`.id=mt_invoice.sims_request_id inner join mt_companies on mt_companies.id=mt_sims_request.company
     where mt_companies.city='$city'   group by mt_sims_request.company ");
...
<br />
   <div id="piechart" ></div><br />
    <?php
    $str='';
    $result_sims = $mysqli->query("SELECT company_name,count(mt_employees.id)as number_of_empolyee from mt_companies left join mt_employees on mt_employees.company=mt_companies.id      where mt_companies.city='$city'    group by company_name");
...
<div id="chart_div" ></div><br />
  ...
  <?php
    $str='';
    $result_sims = $mysqli->query("SELECT city_name,count(mt_employees.id)as number_of_empolyee  from mt_cities left join mt_employees on mt_employees.city=mt_cities.id where mt_cities.id='$city'    group by city_name");
