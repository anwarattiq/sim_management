...
if ($action == "update_$module" || $action == "add_$module") {
	...
	$project_title				= $mysqli->real_escape_string(stripslashes($_POST['project_title']));
	$project_description		= $mysqli->real_escape_string(stripslashes($_POST['project_description']));
...
if ($action == "update_$module" && !empty($id)) {
	...
	if (empty($category) || $category == 'Please select') {
	...
	} else if (empty($project_title)) {
		$error_message = 'Project Title is mandatory.';
	} else if (empty($project_description)) {
		$error_message = 'Project Description is mandatory.';
	} else if (empty($slug)) {
		...
		if ($db_slug != $slug) {
			...
			if (!empty($row_s['id'])) {
				$slug = generate_project_slug($project_title);
	...
	if (empty($project_title)) {
		...
		$insert_row = $mysqli->query("INSERT INTO `$tbl_name`(category, slug, bit, project_title, project_description, meta_title, meta_description, created, publish) VALUES ('" . $categories_ids . "', '" . $slug . "', '" . $bit . "', '" . $project_title . "', '" . $project_description . "', '" . $meta_title . "', '" . $meta_description . "', '" . $created . "', '" . $publish . "'); ");
...
if (!empty($id)) {
	...
	$project_description		= stripslashes($row['project_description']);
					...
					<div class="panel panel-flat">
						<div class="panel-heading">
							...
							<div class="row">
								<div class="col-md-3">
									<div class="col-md-12">
										<h5><?php echo $module_caption; ?> Details <?php if (!empty($slug)) { ?><a href="<?php echo $base_url; ?>/project/<?php echo $slug; ?>" target="_blank">&#128065;</a><?php } ?></h5>
									</div>
								</div>
							...
							<div class="panel-body">
								...
								<div class="form-group">
									...
									<div class="col-lg-9">
										<input name="project_title" id="project_title" value="<?php echo $project_title; ?>" class="form-control" type="text">
									</div>
								</div>
								...
								<div class="form-group">
									...
									<div class="col-lg-9">
										<select class="form-control" name="category[]" id="category[]" multiple="multiple" style="height:400px;">
											...
											<?php
											$result = $mysqli->query("SELECT * FROM `" . $tbl_prefix . "project_categories` WHERE publish=1 ORDER BY category");
								...
								<div class="form-group">
									<label class="col-lg-3 control-label"><strong>Photo[<?php echo $image_width; ?>px x <?php echo $image_height; ?>px]</strong> </label>
									<div class="col-lg-9">
										<input type="file" name="photo" id="photo" class="file-styled">
										<span class="help-block">Size 650 x 500 -> Accepted formats: gif, png, jpg. Max file size 5Mb</span>
									</div>
								</div>
					...
					<div class="panel panel-flat">
						<div class="panel-heading">
							<div class="panel-body">
								...
								<div class="form-group">
									<label><strong>project Description</strong> <span class="f_req">*</span></label>
									<textarea class="sepH_a form-control" rows="3" cols="1" id="project_description" name="project_description"><?php echo $project_description; ?></textarea>
								</div>
